...
Player_2::Player_2(PlayerChanger* changer) : Player(changer, CharaGraphics::GetGraHandle(0, 2)) {
}
...
void Player_2::Initialize() {
	...
	sizeX = 500, sizeY = 283;
	//LoadDivGraph("images/player_2-1-1.png", 24, 2, 12, sizeX, sizeY, charaHandle);
	//GetGraph(0, 2, charaHandle);
	xcount = 0, ycount = 0;
	attackCount = 0;
	godCount = -1;
	jumpFlag = false;
...
void Player_2::Update() {
	if (hp == 0) {
		...
		if (bossFlag == true && bscount <= 1280) {
			...
			if (weaponNum != 2 && weaponNum != 5 && weaponNum != 6) {
				if (weaponNum == 1) {
