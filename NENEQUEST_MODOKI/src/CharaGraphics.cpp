...
void CharaGraphics::Initialize() {	// ‰æ‘œ‚Ì“Ç‚İ‚İ
	...
	LoadDivGraph("images/player1.png", 24, 2, 12, 500, 283, mPl1to3Handle[0]);
	LoadDivGraph("images/player2.png", 24, 2, 12, 500, 283, mPl1to3Handle[1]);
	LoadDivGraph("images/player3.png", 24, 2, 12, 500, 283, mPl1to3Handle[2]);

	// Player4‚Ì‰æ‘œ‚Ì“Ç‚İ‚İ
	LoadDivGraph("images/player4.png", 32, 2, 16, 500, 283, mPl4Handle);

	// Enemy1`2‚Ì‰æ‘œ‚Ì“Ç‚İ‚İ
	LoadDivGraph("images/enemy1.png", 4, 2, 2, 250, 177, mEne1to2Handle[0]);
	LoadDivGraph("images/enemy2.png", 4, 2, 2, 250, 177, mEne1to2Handle[1]);

	// Enemy3`7‚Ì‰æ‘œ‚Ì“Ç‚İ‚İ
	LoadDivGraph("images/enemy3.png", 8, 2, 4, 393, 177, mEne3to7Handle[0]);
	LoadDivGraph("images/enemy4.png", 8, 2, 4, 225, 192, mEne3to7Handle[1]);
	LoadDivGraph("images/enemy5.png", 8, 2, 4, 225, 192, mEne3to7Handle[2]);
	LoadDivGraph("images/enemy6.png", 8, 2, 4, 225, 190, mEne3to7Handle[3]);
	LoadDivGraph("images/enemy7.png", 8, 2, 4, 303, 151, mEne3to7Handle[4]);
...
const int* CharaGraphics::GetGraHandle(const int type, const int charaId) {
	if (type == 0) {	// Player
		if (charaId != 4) {
			return mPl1to3Handle[charaId-1];
		}
		else {
			return mPl4Handle;
		}
		...
		if (charaId <= 2) {
			return mEne1to2Handle[charaId-1];
		}
		else if (charaId <= 7) {
			return mEne3to7Handle[charaId-3];
		}
		else if (charaId == 8){
